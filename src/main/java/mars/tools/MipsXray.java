...
public class MipsXray extends AbstractMarsToolAndApplication {
  ...
  /**
   * Implementation of the inherited abstract method to build the main display area of the GUI. It
   * will be placed in the CENTER area of a BorderLayout. The title is in the NORTH area, and the
   * controls are in the SOUTH area.
   */
  ...
  // Function that gets the current instruction in memory and start animation with the selected
  // instruction.
  ...
  // Internal class that set the parameters value, control the basic behavior of the animation , and
  // execute the animation of the
  ...
  class DatapathAnimation extends JPanel implements ActionListener, MouseListener {
    ...
    public void mousePressed(MouseEvent e) {
      ...
      //	System.out.println("olha, capturado x=" + a.getLocation().getX() + " y = " +
      // a.getLocation().getY());
    }
    ...
    public void loadHashMapValues() {
      importXmlStringData(
          "/MipsXRayOpcode.xml", opcodeEquivalenceTable, "equivalence", "bits", "mnemonic");
      importXmlStringData(
          "/MipsXRayOpcode.xml",
        ...
        for (int i = 0;
          ...
          if (instructionCode.substring(0, 6).equals("000000")) { // R-type instructions
            ...
            // System.out.println("store type");
              ...
              .substring(0, 6)
              .matches("0001[0-1][0-1]")) { // BRANCH type instructions
    ...
    public void setUpInstructionInfo(Graphics2D g2d) {
      ...
      if (instructionCode
          .substring(0, 6)
          .equals("000000")) { // R-type instructions description on screen definition.
        textVariable =
            new TextLayout("REGISTER TYPE INSTRUCTION", new Font("Arial", Font.BOLD, 25), frc);
        ...
        textVariable =
            new TextLayout(
                functionEquivalenceTable.get(instructionCode.substring(26, 32)), font, frc);
        g2d.setColor(Color.BLACK);
        ...
        textVariable =
            new TextLayout(
                registerEquivalenceTable.get(instructionCode.substring(6, 11)), font, frc);
        g2d.setColor(Color.BLACK);
        ...
        textVariable =
            new TextLayout(
                registerEquivalenceTable.get(instructionCode.substring(16, 21)), font, frc);
        g2d.setColor(Color.BLACK);
        ...
        textVariable =
            new TextLayout(
                registerEquivalenceTable.get(instructionCode.substring(11, 16)), font, frc);
        g2d.setColor(Color.BLACK);
        ...
        // instruction mnemonic
        textVariable =
            new TextLayout(opcodeEquivalenceTable.get(instructionCode.substring(0, 6)), font, frc);
        g2d.setColor(Color.cyan);
        textVariable.draw(g2d, 65, 500);
        ...
        textVariable.draw(g2d, 105, 500);
          ...
          .substring(0, 6)
          .matches("100[0-1][0-1][0-1]")) { // load instruction
        textVariable =
            new TextLayout(
        ...
        textVariable =
            new TextLayout(
                "M[ "
                    + registerEquivalenceTable.get(instructionCode.substring(16, 21))
                    + " + "
        ...
        // implement co-processors instruction
          ...
          .substring(0, 6)
          .matches("101[0-1][0-1][0-1]")) { // store instruction
        textVariable =
            new TextLayout("STORE TYPE INSTRUCTION", new Font("Verdana", Font.BOLD, 25), frc);
        g2d.setColor(Color.black);
        textVariable.draw(g2d, 280, 30);
        // opcode label
        textVariable = new TextLayout("opcode", fontTitle, frc);
        ...
        textVariable =
            new TextLayout("BRANCH TYPE INSTRUCTION", new Font("Verdana", Font.BOLD, 25), frc);
        g2d.setColor(Color.black);
        ...
        textVariable =
            new TextLayout("IMMEDIATE TYPE INSTRUCTION", new Font("Verdana", Font.BOLD, 25), frc);
        g2d.setColor(Color.black);
        textVariable.draw(g2d, 250, 30);
    ...
    // set the initial state of the variables that controls the animation, and start the timer that
    ...
    private void initImages() {
      try {
        BufferedImage im =
            ImageIO.read(getClass().getResource(Globals.imagesPath + "datapath.png"));
    ...
    // public boolean printTrackRtoL(int init, int end ,int currentIndex, Graphics2D g2d, Color
    // color, int otherAxis,
    ...
    // public boolean printTrackDtoU(int init, int end ,int currentIndex, Graphics2D g2d, Color
    // color, int otherAxis,
    //		 boolean active, boolean firstInteraction){
    ...
    // public boolean printTrackUtoD(int init, int end ,int currentIndex, Graphics2D g2d, Color
    // color, int otherAxis,
    //		 boolean active,  boolean firstInteraction){
    ...
    public void printTextDtoU(Vertex v) {
      ...
      TextLayout actionInFunctionalBlock =
          new TextLayout(v.getName(), new Font("Verdana", Font.BOLD, 13), frc);
      g2d.setColor(Color.RED);
      ...
      if (instructionCode.substring(0, 6).matches("0001[0-1][0-1]")
          && v.getName().equals("CP+4")) // branch code
      actionInFunctionalBlock =
            new TextLayout("PC+OFFSET", new Font("Verdana", Font.BOLD, 13), frc);
    ...
    // set and execute the information about the current position of each line of information in the
    // animation,
    // verifies the previous status of the animation and increment the position of each line that
    // interconnect the unit function.
      ...
      for (int i = 0; i < vertexTraversed.size(); i++) {
        ...
        if (vert.isMovingXaxis == true) {
          ...
          if (vert.getDirection() == Vertex.movingDownside) {
            if (vert.isText == true) printTextDtoU(vert);
