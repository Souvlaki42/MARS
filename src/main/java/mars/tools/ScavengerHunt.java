...
/**
 * Demo of Mars tool capability. Ken Vollmar, 27 Oct 2006 KenVollmar@missouristate.edu This tool
 * displays movements by a series of players in a game of ScavengerHunt. Players will read and write
 * MIPS memory-mapped locations to move and regain energy. See accompanying documentation for
 * memory-mapped addresses, rules of the game, etc.
 */
public class ScavengerHunt implements Observer, MarsTool {
  ...
  private static final int NUM_PLAYERS =
      22; // Number of players in the game, including "baseline" player
  ...
  private static final int ENERGY_PER_MOVE =
      1; // Energy used in making each move (regardless of distance)
  private static final int SIZE_OF_TASK = 20; // Number of elements in the task

  private static final int NUM_LOCATIONS =
      7; // Number of locations to which ScavengerHunt players travel.
  // The first (n-1) locations are "random" locations, the last is START_AND_END_LOCATION

  private static final int START_AND_END_LOCATION =
      255; // Start and end location of the ScavengerHunt
  private static final int ADMINISTRATOR_ID = 999; // Special ID for administrator

  // MIPS addresses of administrative memory space.
  // The administrator MIPS program writes a value to the Authentication field prior to writing a
  // new
  ...
  // Each change of data in the PlayerID field is a signal for the Tool to check the value of the
  // Authentication
  // field. If the value of the Authentication field is correct, the value of the PlayerID field is
  // used
  // for memory bounds checking. A new value of the Authentication field is expected at each change
  // of the
  // PlayerID field (one-time pad system).
  private static final int ADDR_AUTHENTICATION =
      0xffffe000; // MIPS byte address of Authentication field
  ...
  private static final int ADDR_NUM_TURNS =
      0xffffe00c; // MIPS byte address of number of turns remaining in the game
  ...
  private static final int ADDR_BASE =
      0xffff8000; // MIPS byte address of memory space for first player
  private static final int MEM_PER_PLAYER =
      0x400; // MIPS bytes of memory space given to each player
  ...
  private static int playerID =
      ADMINISTRATOR_ID; // Range 0...(NUM_PLAYERS-1), plus ADMINISTRATOR_ID
  ...
  private class PlayerData {
    ...
    long finishTime;
    // int locID;  // ID of the location to which ScavengerHunt players are headed. Not used by
    // player.
    boolean hasVisitedLoc[] =
        new boolean[NUM_LOCATIONS]; // boolean: player has visited each location
    boolean finis = false;
  ...
  /* ------------------------------------------------------------------------- */
  /**
   * ScavengerHuntDisplay does not have access to the same MIPS Memory class object used by the
   * ScavengerHunt class object. Need read-only access to a similar data structure maintained within
   * ScavengerHunt.
   */
  private class ScavengerHuntDisplay extends JPanel {
    ...
    /**
     * paintComponent does not have access to the same MIPS Memory class object used by the
     * ScavengerHunt class object. Need read-only access to a similar data structure maintained
    ...
    public void paintComponent(Graphics g) {
      ...
      if (!GameOn) // Make sure game is ready before continuing
      {
        ...
        g2.drawString(
            " ScavengerHunt not yet initialized by MIPS administrator program.", 100, 200);
        return;
      }
      ...
      for (int i = 0; i < NUM_LOCATIONS; i++) {
        ...
        g2.drawString(
            " " + i, xCoord + 4, yCoord + 15); // coord is lower left corner of string text box
      ...
      g2.drawLine(
          ...
          35,
          width - 120,
          35 + (NUM_PLAYERS * 15)); // vertical line for location-visited marks
      g2.drawLine(
          width - 50,
          35,
          width - 50,
      ...
      for (int i = 0; i < NUM_PLAYERS; i++) {
        ...
        // ystem.out.println("paintComponent loop " + i + ": Loc coord is (" + xCoord + ", " +
        // yCoord);
        ...
        g2.drawString(
            " " + pd[i].getEnergy(), width - 40, 50 + (i * 15)); // Player's energy on scoreboard

        // Display player's progress or finishing time, whichever is applicable
        if (pd[i].isFinished()) {
          // Display finishing time
          // This doesn't display leading zeroes to align time components (e.g. 27 millisec ought to
          // print as 027)
          g2.drawString(
              pd[i].getFinishMin()
                  + ":"
  ...
  public void update(Observable o, Object arg) {
    ...
    // System.out.println("ScavengerHunt.update: observed write access by player " + playerID + " on
    // Mem[ " +
    ...
    */
    ...
    if (isWrite && playerID == ADMINISTRATOR_ID && address == ADDR_GAME_ON) {
      ...
      // NO ACTION HERE
        ...
        && address
            == ADDR_PLAYER_ID) // if the data written will change the PlayerID, authenticate the
    // write
    {
        ...
        && data != 0) //  Player wrote data to his/her assigned MoveReady location
    {
      ...
      {
        // JOptionPane.showMessageDialog(null, "Player " + playerID + " can't move -- out of
        // bounds.\n" +
        //                                    "(This msg. in ScavengerHunt.update()" );
      ...
      if (Math.sqrt(
        ...
        toolWritePlayerData(
            playerID, OFFSET_WHERE_AM_I_X, toolReadPlayerData(playerID, OFFSET_MOVE_TO_X));
        toolWritePlayerData(
            playerID, OFFSET_WHERE_AM_I_Y, toolReadPlayerData(playerID, OFFSET_MOVE_TO_Y));
        ...
        && data != 0) //  Player wrote data to his/her assigned TaskComplete location
    {
      ...
      //  System.out.println(" ******** ScavengerHunt.update: Player " + playerID + " requests more
      // energy (task complete)" );
    ...
    //  Player wrote data elsewhere within his/her assigned location
    {
      ...
      // System.out.println( "ScavengerHunt.update(): Player " + playerID + " wrote to valid
      // location");
  ...
  private void toolSetWord(int address, int data) {
    ...
    if (KENVDEBUG) {
      System.out.println(
          "   ScavengerHunt.toolSetWord: Setting MIPS Memory["
              + Binary.intToHexString(address)
              + "] to "
    ...
    try {
      ...
      System.out.println(
          "ScavengerHunt.toolSetWord: deliberate exit on " + e.getMessage() + " exception.");
      System.out.println("     SetWordCounter = " + SetWordCounter);
      System.out.println("     address = " + Binary.intToHexString(address));
      System.out.println("     data = " + data);
      System.exit(0);
  ...
  private int toolGetWord(int address) {
    ...
    try {
      ...
      */
      ...
      System.out.println(
          "ScavengerHunt.toolGetWord: deliberate exit on " + e.getMessage() + " exception.");
      System.out.println("     SetWordCounter = " + SetWordCounter);
      System.out.println("     address = " + Binary.intToHexString(address));
      System.exit(0);
    }
  ...
  private int toolReadPlayerData(int p, int offset) {
    ...
    if (KENVDEBUG) {
      ...
      //     (ADDR_BASE + (p * MEM_PER_PLAYER) + offset) < 0xffff8800)  // Show debug for player 0
      // and 1 only
      // {
      ...
      System.out.println(
          "ScavengerHunt.toolReadPlayerData: Mem["
              + Binary.intToHexString(ADDR_BASE + (p * MEM_PER_PLAYER) + offset)
              + "] = "
              + Binary.intToHexString(returnValue)
              + " --- returning normally");
  ...
  private void toolWritePlayerData(int p, int offset, int data) {
    ...
    if (KENVDEBUG) {
      System.out.println(
          "ScavengerHunt.toolWritePlayerData: called with player "
              + p
              + ", offset = "
              + Binary.intToHexString(offset)
              + ", data = "
    ...
    if (KENVDEBUG) {
      ...
      if (data != verifyData) {
        ...
        System.out.println(
            "      actual data at that loc is " + Binary.intToHexString(toolGetWord(address)));
        System.exit(0);
      } else
        System.out.println(
            "  ScavengerHunt.toolWritePlayerData: Mem["
                + Binary.intToHexString(address)
                + " verified as "
                + Binary.intToHexString(data));
    }
  ...
  private void initializeScavengerData() {
    ...
    { // Two coordinates (x and y) for each location
      ...
      loc[j].Y =
          (int)
              (randomStream.nextDouble()
                  * (GRAPHIC_HEIGHT - 50)); // Y coord. (leave room for buttons in window)
    ...
    loc[NUM_LOCATIONS - 1] =
        new Location(); // The last location is a return to the starting position
