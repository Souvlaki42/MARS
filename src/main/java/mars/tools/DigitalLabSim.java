    ...
    if (CounterInterruptOnOff)
      if (CounterValue > 0) {
        ...
        if ((Coprocessor0.getValue(Coprocessor0.STATUS) & 2) == 0) {
          mars.simulator.Simulator.externalInterruptingDevice = /*Exceptions.*/
  ...
  public void updateHexaKeyboard(char row) {
    ...
    if ((key != -1) && ((1 << (key / 4)) == (row & 0xF))) {
      updateMMIOControlAndData(
          OUT_ADRESS_HEXA_KEYBOARD, (char) (1 << (key / 4)) | (1 << (4 + (key % 4))));
    } else {
      updateMMIOControlAndData(OUT_ADRESS_HEXA_KEYBOARD, 0);
    }
    if ((row & 0xF0) != 0) KeyboardInterruptOnOff = true;
    else KeyboardInterruptOnOff = false;
  }
