...
public class MarsBot implements Observer, MarsTool {
  ...
  private boolean MarsBotLeaveTrack =
      false; // true --> leave track when moving, false --> do not ...
  ...
  private final int trackPts =
      256; // TBD Hardcoded. Array contains start-end points for segments in track.
    ...
    public void run() {
      ...
      do {
        if (MarsBotMoving) {
          ...
          MarsBotXPosition +=
              Math.cos(Math.toRadians(tempAngle)); // Math.cos parameter unit is radians
          MarsBotYPosition +=
              -Math.sin(Math.toRadians(tempAngle)); // Negate value because Y coord grows down
          ...
          // System.out.println(" ------- Heading is " + MarsBotHeading + ", angle is " +
          // tempAngle);
  ...
  private class MarsBotDisplay extends JPanel {
    ...
    public void paintComponent(Graphics g) {
      ...
      */
      ...
      g2.fillRect(
          (int) MarsBotXPosition,
          (int) MarsBotYPosition,
          20,
          20); // Draw bot at its current position
    ...
    if (arg instanceof MemoryAccessNotice) {
      ...
      if (address < 0 && notice.getAccessType() == AccessNotice.WRITE) {
        ...
        if (address == ADDR_HEADING) {
          ...
          // message = "MarsBot.update: HEY!!! unknown address of " + Integer.toString(address) + ",
          // value: ";
          // System.out.println(message + notice.getValue() );
        }
      }
