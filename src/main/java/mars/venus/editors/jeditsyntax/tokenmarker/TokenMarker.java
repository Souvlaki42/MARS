 ...
 *
 * <p>For performance reasons, the linked list of tokens is reused after each line is tokenized.
...
public abstract class TokenMarker {
  ...
  /**
   * An abstract method that splits a line up into tokens. It should parse the line, and call <code>
   * addToken()</code> to add syntax tokens to the token list. Then, it should return the initial
   * token type for the next line.
   *
   * <p>For example if the current line contains the start of a multiline comment that doesn't end
   * on that line, this method should return the comment token type so that it continues on the next
   * line.
   *
   ...
   * object using this token marker is required to pass all lines in the document to the <code>
   * markTokens()</code> method (in turn).
   *
   * <p>The default implementation returns true; it should be overridden to return false on simpler
  ...
  /**
   * Construct and return any appropriate help information for the given token. This default
   * definition returns null; override it in language-specific subclasses.
   *
   ...
   * Will match instruction prefixes, e.g. "s" matches "sw". This default definition returns null;
  ...
  public List<PopupHelpItem> getTokenPrefixMatchHelp(
      String line, Token tokenList, Token tokenAtOffset, String tokenText) {
    return null;
  }
  ...
  /**
   * The first token in the list. This should be used as the return value from <code>markTokens()
   * </code>.
   */
