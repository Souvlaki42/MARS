...
public class MIPSTokenMarker extends TokenMarker {
    ...
    if (token != null && token.id == Token.KEYWORD1) {
      ...
      if (instrMatches.size() > 0) {
        ...
        for (Instruction inst : instrMatches) {
          if (mars.Globals.getSettings().getExtendedAssemblerEnabled()
              || inst instanceof BasicInstruction) {
            matches.add(
                new PopupHelpItem(tokenText, inst.getExampleFormat(), inst.getDescription()));
            realMatches++;
          }
        }
        if (realMatches == 0) {
    ...
    if (token != null && token.id == Token.KEYWORD2) {
      ...
      if (dir != null) {
        return Collections.singletonList(
            new PopupHelpItem(tokenText, dir.getName(), dir.getDescription()));
      }
    }
    return matches;
  }
  ...
  /**
   * Construct and return any appropriate help information for prefix match based on current line's
   * token list.
   *
   ...
   * @return List of PopupHelpItem objects, one per match.
   */
  public List<PopupHelpItem> getTokenPrefixMatchHelp(
      String line, Token tokenList, Token token, String tokenText) {
    ...
    // CASE:  Current token is valid KEYWORD1 (MIPS instruction).  If this line contains a previous
    // KEYWORD1 or KEYWORD2
    //        token, then we ignore this one and do exact match on the first one.  If it does not,
    // there may be longer
    //        instructions for which this is a prefix, so do a prefix match on current token.
    if (token != null && token.id == Token.KEYWORD1) {
      if (moreThanOneKeyword) {
        return (keywordType == Token.KEYWORD1)
            ? getTextFromInstructionMatch(keywordTokenText, true)
    ...
    // CASE:  Current token is valid KEYWORD2 (MIPS directive).  If this line contains a previous
    // KEYWORD1 or KEYWORD2
    //        token, then we ignore this one and do exact match on the first one.  If it does not,
    // there may be longer
    ...
    // CASE:  Current token is NULL, which can be any number of things.  Think of it as being either
    // white space
    //        or an in-progress token possibly preceded by white space.  We'll do a trim on the
    // token.  Now there
    //        are two subcases to consider:
    //    SUBCASE: The line does not contain any KEYWORD1 or KEYWORD2 tokens but nothing remains
    // after trimming the
    //             current token's text.  This means it consists only of white space and there is
    // nothing more to do
    //             but return.
    //    SUBCASE: The line does not contain any KEYWORD1 or KEYWORD2 tokens.  This means we do a
    // prefix match of
    ...
    if (token != null && token.id == Token.NULL) {
      ...
      // Subcase: no KEYWORD1 or KEYWORD2 but current token contains nothing but white space.  We're
      // done.
      if (keywordTokenText == null && trimmedTokenText.length() == 0) {
        return null;
      }

      // Subcase: no KEYWORD1 or KEYWORD2.  Generate text based on prefix match of trimmed current
      // token.
      if (keywordTokenText == null && trimmedTokenText.length() > 0) {
        if (trimmedTokenText.charAt(0) == '.') {
          return getTextFromDirectiveMatch(trimmedTokenText, false);
        } else if (trimmedTokenText.length()
            >= mars.Globals.getSettings().getEditorPopupPrefixLength()) {
  ...
  private List<PopupHelpItem> getTextFromInstructionMatch(String tokenText, boolean exact) {
    ...
    for (Instruction inst : matches) {
      if (mars.Globals.getSettings().getExtendedAssemblerEnabled()
        ...
        if (exact) {
          results.add(
              new PopupHelpItem(tokenText, inst.getExampleFormat(), inst.getDescription(), exact));
        } else {
   ...
   *
   * @return KeywordMap where key is the keyword and associated value is the token type (e.g.
   *     Token.KEYWORD1).
   */
  public static KeywordMap getKeywords() {
    if (cKeywords == null) {
      ...
      // Note: Coprocessor 0 registers referenced only by number: $8, $12, $13, $14. These are
      // already in the map
