...
/**
 * An input handler converts the user's keystrokes into concrete actions. It also takes care of
 * macro recording and action repetition.
 *
 * <p>This class provides all the necessary support code for an input handler, but doesn't actually
 * do any key binding logic. It is up to the implementations of this class to do so.
 *
...
public abstract class InputHandler extends KeyAdapter {
  /**
   * If this client property is set to Boolean.TRUE on the text area, the home/end keys will support
   * 'smart' BRIEF-like behaviour (one press = start/end of line, two presses = start/end of
   * viewscreen, three presses = start/end of document). By default, this property is not set.
   */
  ...
  /**
   * Returns a copy of this input handler that shares the same key bindings. Setting key bindings in
   * the copy will also set them in the original.
   */
  ...
  /**
   * If an action implements this interface, it should not be repeated. Instead, it will handle the
   * repetition itself.
   */
  ...
  public static class next_page implements ActionListener {
    ...
    public void actionPerformed(ActionEvent evt) {
      ...
      int caret =
          textArea.getLineStartOffset(Math.min(textArea.getLineCount() - 1, line + visibleLines));
      if (select) textArea.select(textArea.getMarkPosition(), caret);
      else textArea.setCaretPosition(caret);
    }
  }
