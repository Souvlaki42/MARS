...
public class JEditTextArea extends JComponent {
  /**
   * Adding components with this name to the text area will place them left of the horizontal scroll
   * bar. In jEdit, the status bar is added this way.
   */
  public static String LEFT_OF_SCROLLBAR = "los";

  /**
   * Get the popup help text color from the current UIManager theme. Falls back to black if not
   * available.
   *
  ...
  public JEditTextArea(TextAreaDefaults defaults, JComponent lineNumbers) {
    ...
    JScrollPane lineNumberScroller =
        new JScrollPane(
            lineNumbers,
  ...
  public void updateScrollBars() {
    if (vertical != null && visibleLines != 0) {
      ...
      // on pixels, so I need to adjust accordingly to keep it in synch with the editing area
      // scrollbar.
      // DPS 4-May-2010
      int height = painter.getFontMetrics(painter.getFont()).getHeight();
      lineNumbersVertical.setValues(
          firstLine * height, visibleLines * height, 0, getLineCount() * height);
  ...
  /**
   * Converts an offset in a line into an x co-ordinate. This is a slow version that can be used any
   * time.
   *
   * @param line The line
  ...
  /**
   * Returns the document's token marker. Equivalent to calling <code>getDocument().getTokenMarker()
   * </code>.
   */
  ...
  /**
   * Sets the document's token marker. Equivalent to caling <code>getDocument().setTokenMarker()
   * </code>.
   *
  ...
  /**
   * Returns the length of the document. Equivalent to calling <code>getDocument().getLength()
   * </code>.
   */
  ...
  /**
   * Copies the text on the specified line into a segment. If the line is invalid, the segment will
  ...
  /**
   * Sets the selection end. The new selection will be the old selection start and the new selection
   * end.
   *
  ...
  class ScrollLayout implements LayoutManager {
    public void addLayoutComponent(String name, Component comp) {
      if (name.equals(CENTER)) center = comp;
      else if (name.equals(RIGHT)) right = comp;
      else if (name.equals(BOTTOM)) bottom = comp;
    ...
    public void mouseWheelMoved(MouseWheelEvent e) {
      ...
      if (e.getWheelRotation() < 0) {
        ...
        setFirstLine(
            getFirstLine()
                + (Math.min(
                    maxMotion, Math.max(0, getLineCount() - (getFirstLine() + visibleLines)))));
      }
    }
  }
  ...
  public String getSyntaxSensitiveToolTipText(int x, int y) {
    ...
    List<PopupHelpItem> matches =
        getSyntaxSensitiveHelpAtLineOffset(line, this.xToOffset(line, x), true);
    if (matches == null) {
      return null;
    }
    ...
    for (PopupHelpItem match : matches) {
      result +=
          glue
              + "<tt>"
              + match.getExamplePaddedToLength(length).replaceAll(" ", "&nbsp;")
              + "</tt>"
  ...
  /**
   * Constructs string for auto-indent feature. Returns empty string if auto-intent is disabled or
   * if line has no leading white space. Uses getLeadingWhiteSpace(). Is used by InputHandler when
   * processing key press for Enter key. DPS 31-Dec-2010
   *
   * @return String containing auto-indent characters to be inserted into text
   */
  public String getAutoIndent() {
    return (Globals.getSettings().getBooleanSetting(Settings.AUTO_INDENT))
        ? getLeadingWhiteSpace()
        : "";
  }

  /**
   * Makes a copy of leading white space (tab or space) from the current line and returns it. DPS
   * 31-Dec-2010
   *
  ...
  private List<PopupHelpItem> getSyntaxSensitiveHelpAtLineOffset(
      int line, int offset, boolean exact) {
    ...
    if (tokenMarker != null) {
      ...
      if (tokenAtOffset != null) {
        String tokenText =
            lineSegment.toString().substring(tokenOffset, tokenOffset + tokenAtOffset.length);
        if (exact) {
          return tokenMarker.getTokenExactMatchHelp(tokenAtOffset, tokenText);
        } else {
          return tokenMarker.getTokenPrefixMatchHelp(
    ...
    if (helpItems != null) {
      ...
      for (PopupHelpItem item : helpItems) {
        JMenuItem menuItem =
            new JMenuItem(
                "<html><tt>"
                    + item.getExamplePaddedToLength(length).replaceAll(" ", "&nbsp;")
                    + "</tt>"
                    + item.getDescription()
                    + "</html>");
        if (item.getExact()) {
          ...
          menuItem.addActionListener(
              new PopupHelpActionListener(item.getTokenText(), item.getExample()));
        }
