...
/**
 * <code>RepeatButton</code> is a <code>JButton</code> which contains a timer for firing events
 * while the button is held down. There is a default initial delay of 300ms before the first event
 * is fired and a 60ms delay between subsequent events. When the user holds the button down and
 * moves the mouse out from over the button, the timer stops, but if the user moves the mouse back
 * over the button without having released the mouse button, the timer starts up again at the same
 * delay rate. If the enabled state is changed while the timer is active, it will be stopped.
 *
 * <p>NOTE: The normal button behavior is that the action event is fired after the button is
 * released. It may be important to konw then that this is still the case. So in effect, listeners
 * will get 1 more event then what the internal timer fires. It's not a "bug", per se, just
 * something to be aware of. There seems to be no way to suppress the final event from firing
 * anyway, except to process all ActionListeners internally. But realistically, it probably doesn't
 * matter.
 */
public class RepeatButton extends JButton implements ActionListener, MouseListener {
  ...
  /**
   * Flag to indicate that the button should fire events when held. If false, the button is
   * effectively a plain old JButton, but there may be times when this feature might wish to be
   * disabled.
   */
  ...
  /**
   * Holder of the modifiers used when the mouse pressed the button. This is used for subsequently
   * fired action events. This may change after mouse pressed if the user moves the mouse out,
   * releases a key and then moves the mouse back in.
   */
  ...
  /**
   * Checks if the button should fire events when held. If false, the button is effectively a plain
   * old JButton, but there may be times when this feature might wish to be disabled.
   *
  ...
  /**
   * Sets if the button should fire events when held. If false, the button is effectively a plain
   * old JButton, but there may be times when this feature might wish to be disabled. If false, it
   * will also stop the timer if it's running.
   *
  ...
  public void actionPerformed(ActionEvent ae) {
    ...
    if (ae.getSource() == this.timer) {
      ActionEvent event =
          new ActionEvent(
              this, ActionEvent.ACTION_PERFORMED, super.getActionCommand(), this.modifiers);
